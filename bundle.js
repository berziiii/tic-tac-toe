...
/***/ function(module, exports, __webpack_require__) {
	...
	var displayAlert = function displayAlert(alert) {
	...
	var showAuthView = function showAuthView() {
	...
	var showGameView = function showGameView() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var updateGame = function updateGame(index, value, done) {
	  api.patchGame(index, value, done).done(ui.updateGameSuccess).fail(ui.updateGameSuccess);
	};
	...
	var onShowProfile = function onShowProfile(event) {
	  ...
	  api.indexGames().done(ui.showProfile).fail(ui.indexGamesFailure);
	};

	// Switches to the game status view when the "Game" button is clicked
	var onShowGameStatus = function onShowGameStatus() {
	...
	var onClickCell = function onClickCell(event) {
	  ...
	  var turnResult = logic.processTurn(index);
	...
	var addCellHandlers = function addCellHandlers(numCells) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var paintCell = function paintCell(value, index) {
	  var id = index + 1;
	  var cellId = '#cell' + id;
	  $(cellId).text(value);
	};
	...
	var paintBoard = function paintBoard(cells) {
	...
	var displayWhoseTurn = function displayWhoseTurn() {
	  $('#active-player').text("player " + app.activePlayer + '\'s');
	};

	// Changes the active player
	var changeWhoseTurn = function changeWhoseTurn() {
	...
	var initializeGameView = function initializeGameView() {
	  resetAlert();
	  displayWhoseTurn();
	  $('#win-message').hide();
	  $('#whose-turn').show();
	...
	var showGameStatusView = function showGameStatusView() {
	  resetAlert();
	...
	var showProfileView = function showProfileView() {
	  resetAlert();
	  $('#game-status-view').hide();
	  $('#profile-view').show();
	};
	...
	var showProfile = function showProfile(data) {
	  app.user.games = data.games;
	  var games = app.user.games;
	...
	module.exports = {
	  ...
	  showGameStatusView: showGameStatusView,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var isLegalMove = function isLegalMove(index) {
	  var game = app.user.game;
	...
	var isTieGame = function isTieGame(cells) {
	  if (cells.indexOf('') === -1) {
	    return true;
	  }
	};
	...
	var isOver = function isOver(index, value) {
	  ...
	  var cells = app.user.game.cells.slice();

	  // apply the most recent play to our copied array
	  cells[index] = value;

	  // check if that play results in a win
	  if (cells[0] === cells[1] && cells[0] === cells[2] && cells[0] !== '' || cells[3] === cells[4] && cells[3] === cells[5] && cells[3] !== '' || cells[6] === cells[7] && cells[6] === cells[8] && cells[6] !== '' || cells[0] === cells[3] && cells[0] === cells[6] && cells[0] !== '' || cells[1] === cells[4] && cells[1] === cells[7] && cells[1] !== '' || cells[2] === cells[5] && cells[2] === cells[8] && cells[2] !== '' || cells[0] === cells[4] && cells[0] === cells[8] && cells[0] !== '' || cells[2] === cells[4] && cells[2] === cells[6] && cells[2] !== '') {
	...
	var processTurn = function processTurn(index) {
	  // is it a legal move?
